...
import { EmptyState, Field, groupLabel, PageContainer, SectionCard, sideLabel, SmartNumberInput, StatusPill } from '../app/ui';
...
export function InvitationsPage() {
  ...
  const handleWhatsappLink = async (id: string) => {
    try {
      ...
      setNotice('קישור הוואטסאפ הועתק ללוח.');
    } catch (err) {
  ...
  return (
    <PageContainer
      title="הזמנות ומשקי בית"
      subtitle="כאן מתחיל כל ה-flow הציבורי: RSVP, WhatsApp, שיוך צד וקבוצות."
      loading={loading}
      error={error}
    >
      <div className="grid gap-6 xl:grid-cols-[0.78fr_1.22fr]">
        <SectionCard title="הזמנה חדשה" kicker="Household record">
          <form className="space-y-4" onSubmit={handleCreate}>
            <Field label="שם משק בית">
              <input className="input" value={form.householdName} onChange={(event) => setForm({ ...form, householdName: event.target.value })} required />
            </Field>
            <Field label="מספר וואטסאפ">
              <input className="input" dir="ltr" value={form.whatsappNumber} onChange={(event) => setForm({ ...form, whatsappNumber: event.target.value })} />
            </Field>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="סכום מתנה">
                <SmartNumberInput className="input" value={form.giftAmount} onChange={(giftAmount) => setForm({ ...form, giftAmount: Number(giftAmount) })} />
              </Field>
            </div>
        ...
        <SectionCard title="רשימת הזמנות" kicker={`${invitations.length} households`}>
          ...
          {invitations.length === 0 ? (
            <EmptyState title="אין עדיין הזמנות" text="התחילו ממשק בית ראשון או ייבאו רשימה מאקסל." />
          ) : (
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div>
                        ...
                        <p className="mt-2 text-sm text-stone-500">
                          {sideLabel(invitation.side)} · {groupLabel(invitation.group)} · {phone.label}
                        </p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button className="btn btn-secondary" disabled={!phone.valid} onClick={() => handleWhatsappLink(invitation._id)}>
                          קישור RSVP
                        </button>
                        <button className="btn btn-danger" onClick={() => handleDelete(invitation._id)}>
                          מחיקה
                        </button>
                      </div>
                    </div>
